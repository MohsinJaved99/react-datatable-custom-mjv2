...
const DataTable = ({
    ...
    const localData = localStorage.getItem(key) ? localStorage.getItem(key).split(',').map((el) => el === 'true') : null;
    const [isHideColumnsVisible, setIsHideColumnsVisible] = useState(false);
    const [visibleColumns, setVisibleColumns] = useState(columnsVisibility ? (localData ? localData : columns.map(() => true)) : columns.map(() => true));
    ...
    useEffect(() => {
        if (checkbox) {
            const cb = data.map((item, index) => ({
                index: index,
                value: checkboxValueSelector.split('.').reduce((acc, key) => acc[key], item),
                checked: false
            }));
            setCheckboxes(cb);
        }
                ...
                <p>Rows per page:
                    ...
                    >
                        {pageSizeOptions.map((pagination, index) => (
                            <option key={index} value={pagination}>{pagination}</option>
                        ))}
                    </select>
                </p>
            </div>
    ...
    const PaginationInformation = () => {
        ...
        return (
            <div className={"w-[fit-content]"}>
                <p><span className={"font-semibold"}>{offset}</span> - <span className={"font-semibold"}>{limit}</span> of <span className={"font-semibold"}>{totalData}</span></p>
            </div>
        );
    };

    const PaginationButtons = () => {
        const className = 'flex items-center justify-center rounded-full bg-gray-100 w-8 h-8 enabled:hover:bg-gray-200 disabled:opacity-[0.5]';
    ...
    const CheckboxSelectedCount = () => {
        return (checkbox && checkboxes.some(checkbox => checkbox.checked) && (
            <div className={"fixed flex bg-[rgba(0,0,0,0.2)] rounded bottom-[80px] text-center font-semibold right-10 w-[fit-content] p-2 cursor-pointer transition-all duration-700 hover:hidden"}>
                {checkboxes.filter(checkbox => checkbox.checked).length} rows selected
            </div>
        ));
    };
                ...
                const rowData = columns.map(col => {
                    ...
                    if (typeof value === 'object' && value?.props?.children) {
                        const children = Array.isArray(value.props.children)
                            ...
                            : [value.props.children];
                        return children.map(child => typeof child === 'object' ? '' : child)
                            .join(' ')
                            .replace(/\s{2,}/g, ' ')
                            .replace(/,/g, ';');
                    }
        ...
        <div className={"datatable"}>
            <div className={"w-full"}>
                ...
                <div className={'grid md:grid-cols-2 mt-3'}>
                    ...
                    <div className={'w-full flex justify-end grid grid-cols-1 gap-1 md:grid-cols-2'}>
                        <div className={'flex justify-end gap-2'}>
                            {
                                exportable && (
                                    <button title={'Export csv'} disabled={progressPending} onClick={handleExport} className={'flex border-2 border-solid rounded p-2 pl-3 w-fit cursor-pointer border-gray-200 transition-all delay-100 hover:border-gray-500 disabled:opacity-50'}>
                                        Export <Icon className={'mt-[2px] ml-2'} fontSize={'20px'} icon={'prime:file-export'} />
                                    </button>
                                )
                            }

                            {
                                columnsVisibility && (
                                    <div className={'relative'} ref={componentRef}>
                                        <button disabled={progressPending} onClick={handleHideColumnsVisibility} title={'Set column visibility'} className={'flex border-2 border-solid rounded p-2 pl-3 w-fit cursor-pointer border-gray-200 transition-all delay-100 hover:border-gray-500 disabled:opacity-50'}>Columns <Icon className={'mt-1 ml-2'} fontSize={'20px'} icon={'material-symbols:keyboard-arrow-down'} /></button>
                                        <div className={`w-[200px] md:w-[400px] bg-gray-100 p-2 absolute top-12 left-0 z-50 rounded grid md:grid-cols-2 ${isHideColumnsVisible?'block':'hidden'}`}>
                                            {columns.map((column, index) => (
                                                <div key={index} className={'text-xs font-medium uppercase mt-1 mb-1'}>
                                                    <input
                                                        key={index}
                                                        onChange={(e) => handleColumnVisibility(index, e.target.checked)}
                                                        checked={visibleColumns[index]}
                                                        id={column.header}
                                                        type={'checkbox'}
                                                    />
                                                    <label className={'cursor-pointer ml-1'} htmlFor={column.header}>{column.header}</label>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                        ...
                        <div className={'relative w-full'}>
                            <Icon icon={'material-symbols:search'} className={'absolute top-2 left-2 text-[25px] text-gray-400'} />
                            <input
                                type={"text"}
                                value={inputValue}
                ...
                {progressPending ? (
                    <div className="pt-10 pb-10">
                        {progressComponent || <h4 className="w-full text-lg text-center">Loading Data</h4>}
                    </div>
                ) : data.length === 0 ? (
                    <div className="pt-10 pb-10">
                        {noDataComponent || <h4 className="w-full text-lg text-center">No Data Found</h4>}
                    </div>
                ) : (
                    <>
                        <div className={"relative overflow-hidden"}>
                            <div className={`w-full overflow-x-auto scroll-sm ${overflowXScrollOnTop ? 'datatable-scroll-top' : ''}`}>
                                <table className={`w-full mt-4 ${overflowXScrollOnTop ? 'table-scroll-top' : ''}`}>
                                    <thead>
                                    <TableHeader
                                        columns={columns}
                                        checkbox={checkbox}
