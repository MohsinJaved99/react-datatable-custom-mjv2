...
const DataTable = ({
    ...
    if (columnsVisibility) {
      if (localData) {
        if (localData.length !== visibleColumns.length) {
          localStorage.setItem(key, columns.map(() => true));
        }
      } else {
  ...
  const handleExport = () => {
    if (columns.length > 0 && data.length > 0) {
      const header = columns.filter(column => column.export === undefined || column.export === true).map((column) => column.header).join(",") + "\n";

      const rows = data
          .map((item) => {
            const rowData = columns.filter(column => column.export === undefined || column.export === true).map((col) => {
